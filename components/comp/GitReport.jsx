...
<head>
  ...
  <style>
    ...
    .commit-header { display: flex; justify-content: space-between; margin-bottom: 8px; }
    .message { font-weight: 600; font-size: 15px; }
    .meta { font-size: 12px; color: #6b7280; }
    .stats { font-size: 13px; margin-bottom: 8px; }
    .added { color: #16a34a; } .deleted { color: #dc2626; }
  ...
  ${commits.map((c) => `
    <div class="commit">
      <div class="commit-header">
        ...
        <span class="meta">${c.sha}</span>
      </div>
      <div class="meta">${c.author} · ${new Date(c.date).toLocaleString("ru-RU")}</div>
      ${c.stats ? `<div class="stats">
        Файлов: <b>${c.files.length}</b> &nbsp;
        <span class="added">+${c.stats.additions}</span> &nbsp;
        <span class="deleted">-${c.stats.deletions}</span>
      </div>` : ""}
      <table>
