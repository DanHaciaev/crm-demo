...
function EditableRow({ children, onEdit }) {
  return (
    <div className="flex items-center justify-between group">
      <span className="text-xs text-gray-600">{children}</span>
      <button
        onClick={onEdit}
        className="text-xs text-gray-300 group-hover:text-gray-500 transition ml-2"
      >
        ✏️
      </button>
    </div>
...
export default function Charts() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      const [{ data: sales, error: salesError }, { data: traffic, error: trafficError }] =
        await Promise.all([
          client.from("sales").select("*").order("id", { ascending: true }),
          client.from("traffic_sources").select("*").order("id", { ascending: true }),
        ]);

      if (salesError || trafficError) {
        setError((salesError || trafficError).message);
        setLoading(false);
        return;
      }
  ...
  function openCreateSale() {
    ...
    setSaleModal({ mode: "create" });
  }

  function openEditSale(item) {
    setSaleForm({ month: item.month, revenue: item.revenue, expenses: item.expenses });
  ...
  async function handleSaleSave() {
    setSaving(true);
    const payload = {
      month:    saleForm.month,
      revenue:  Number(saleForm.revenue),
      expenses: Number(saleForm.expenses),
    };

    if (saleModal.mode === "create") {
      const { data, error } = await client.from("sales").insert([payload]).select().single();
      if (error) { alert("Ошибка: " + error.message); setSaving(false); return; }
      setSalesData((prev) => [...prev, data]);
    } else {
      const { error } = await client.from("sales").update(payload).eq("id", saleModal.item.id);
      if (error) { alert("Ошибка: " + error.message); setSaving(false); return; }
  ...
  function openCreateTraffic() {
    ...
    setTrafficModal({ mode: "create" });
  }

  function openEditTraffic(item) {
    setTrafficForm({ name: item.name, value: item.value });
  ...
  async function handleTrafficSave() {
    ...
    if (trafficModal.mode === "create") {
      const { data, error } = await client.from("traffic_sources").insert([payload]).select().single();
      if (error) { alert("Ошибка: " + error.message); setSaving(false); return; }
      setPieData((prev) => [...prev, data]);
    } else {
      const { error } = await client.from("traffic_sources").update(payload).eq("id", trafficModal.item.id);
      if (error) { alert("Ошибка: " + error.message); setSaving(false); return; }
  ...
  const pieChartData = pieData.map((t) => ({
    name: t.name,
    value: t.value,
  }));
    ...
    <>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className="px-3 py-1 text-xs rounded-lg bg-black text-white hover:bg-gray-800 transition"
            >
              + Добавить
            </button>
          </div>
      ...
      {trafficModal && (
        <Modal
          ...
          form={trafficForm}
          setForm={setTrafficForm}
          loading={saving}
          onClose={() => setTrafficModal(null)}
          onSubmit={handleTrafficSave}
        />
      )}
    </>
  );
